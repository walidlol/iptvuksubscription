...
const BASE =
  "inline-flex items-center justify-center gap-2 backdrop-blur-glass border rounded-full shadow-glass transition-colors duration-200 cursor-pointer select-none whitespace-nowrap hover:bg-[rgba(255,255,255,0.18)] hover:border-[rgba(255,255,255,0.35)] hover:shadow-glass-hover";

function isValidUKPhone(phone: string): boolean {
  // UK numbers: starts with 44 or 07, 10-11 digits after country code
...
export default function WhatsAppGate({
  ...
  const handleSubmit = useCallback(() => {
    ...
    handleClose();
    setPhone("");
  }, [phone, message, handleClose]);

  const buttonClasses = [BASE, SIZE_CLASSES[size], VARIANT_CLASSES[variant], className].join(" ");
          ...
          <>
            ...
            >
              ...
              >
                ...
                {/* Error */}
                {error && (
                  <p className="text-xs text-[#E8392A] mb-3">{error}</p>
                )}
                ...
                <p className="text-xs text-[#6E6E7A] text-center mt-3">
                  UK numbers only (+44)
                </p>
              </div>
            </motion.div>
